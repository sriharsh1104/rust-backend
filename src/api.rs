...
use mongodb::bson::{doc, oid::ObjectId, Document}; // Import ObjectId explicitly
...
fn verify_jwt_token(token: &str) -> Result<UserClaims, jsonwebtoken::errors::Error> {
    ...
    decode::<UserClaims>(token, &key, &Validation::default()).map(|data| data.claims)
}
...
async fn make_user_admin(email: web::Path<String>, db: web::Data<Database>) -> impl Responder {
    ...
    match collection.update_one(filter, update, None).await {
        ...
        Err(_) => HttpResponse::InternalServerError().json(ApiResponse {
            message: "Error updating user role".to_string(),
            token: None,
        }),
    }
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
    cfg.service(web::resource("/register").route(web::post().to(register)))
        ...
        .service(web::resource("/insert_patent_data").route(web::post().to(insert_patent_data)))
        .service(web::resource("/verify_token/{token}").route(web::get().to(verify_token)))
        .route("/search", web::get().to(search_patents))
        .service(web::resource("/make_user_admin/{user_id}").route(web::put().to(make_user_admin)));
}
